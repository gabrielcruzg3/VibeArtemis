                 ...
                 QString& rtspSessionUrl)
{
    ...
    QString response =
            openConnectionToString(m_BaseUrlHttps,
                                   ...
                                   QString("&vdisplay=1&vdisplay_res=%1x%2&vdisplay_fps=%3&vdisplay_scale=1.00")
                                       .arg(streamConfig->width).arg(streamConfig->height).arg(streamConfig->fps)+
...
bool NvHTTP::uploadClipboard(const QString& text)
{
    QUrl url(m_BaseUrlHttps);
    url.setPath("/actions/clipboard");
    url.setQuery("type=text&uniqueid=" + IdentityManager::get()->getUniqueId());
...
bool NvHTTP::executeServerCommand(uint8_t cmdId)
{
    QUrl url(m_BaseUrlHttps);
    url.setPath("/actions/servercmd");
    url.setQuery(QString("cmd=%1&uniqueid=%2").arg(cmdId).arg(IdentityManager::get()->getUniqueId()));
...
bool NvHTTP::setBitrate(uint32_t bitrateKbps)
{
    QString res = openConnectionToString(m_BaseUrlHttps,
                                         "bitrate",
...
bool NvHTTP::isApolloServer(const QString& serverInfo)
{
    if (serverInfo.contains("apollo", Qt::CaseInsensitive) ||
        serverInfo.contains("vibepollo", Qt::CaseInsensitive) ||
        !getXmlString(serverInfo, "apolloversion").isEmpty()) {
        return true;
    }
    return false;
}
